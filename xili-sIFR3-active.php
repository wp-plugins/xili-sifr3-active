	...
	/* js files in header */
		if (is_sifr3()) {
			...
			if (file_exists(SIFR3INCURTHEMEPATH.'/'.SIFR3SUBJSFOLDER.'/sifr-config.js')) {
				...
				wp_enqueue_script('sifr3c',get_bloginfo('template_directory').'/'.SIFR3SUBJSFOLDER.'/sifr-config.js',false,'3');
			} else {
	...
	function add_sifr3_stylesheet(){
		if (is_sifr3()) {
    		...
    		if (file_exists($ThemeStyleFilePath)){
        		wp_register_style('sifr3StyleSheet', get_bloginfo('template_directory').'/'.SIFR3SUBSIFRFOLDER.'/sifr.css',false,'2.7','screen'); 
...
<div class='wrap'>
		...
		<p>-&nbsp;<?php _e("The current theme is here","xili-sifr3-active"); echo ": <em>".get_bloginfo('template_directory') ?>. </em><br /><br />-&nbsp;
		...
		if (file_exists($themejsfolder.'sifr-config.js')) {
			echo ": <em>".SIFR3INCURTHEMEDIR.'/'.SIFR3SUBJSFOLDER.'/' ;}
		else {
		...
		if (file_exists($themecssfolder.'sifr.css')) {
			echo ": <em>".SIFR3INCURTHEMEDIR.'/'.SIFR3SUBSIFRFOLDER.'/' ;}
		else {
			echo ": <em>".SIFR3DIR.'/css/';
			}		?>. </em><br /><br />
